...
class Pilha {
private:
    T* _dados;       // vetor dinamico
    int _topo;         // indicador do topo
    int _tam;          // indica tamanho da pilha

public:
    // Método construtor da pilha
    Pilha(int tam) {
        _tam = tam;
        _dados = new T[tam];      // Aloca memoria dinamicamente
...
int main(){
    ...
    int n;
    cout << "Digite o Tamanho Maximo da Pilha: ";
    cin >> n;
    
    //SE QUISER MUDAR O TIPO DE DADOS DA PILHA É SO ALTERAR AQUI O TIPO E NO VALOR DENTRO DO TRY
    Pilha<int> pilha_1(n);
    while (1)
    {
